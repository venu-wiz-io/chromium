...
// TODO
    ...
    virtual Status   OpenMic(const std::string& id) = 0;
    ...
    using AudioStreamCB =
        base::Callback<void(const scoped_refptr<media::AudioInputStream>& )>;

    virtual void StartAudioCapture(
        const std::string& id,
        const AudioStreamCB& audio_stream_callback) = 0;
    ...
    virtual Status   SetMicParameters(const std::string& id, const AudioParameters& parameters) = 0;
#endif
