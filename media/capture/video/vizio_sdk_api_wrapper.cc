                   ...
                   const viziosdk::duration_nanos timestamp)
{
    ...
    client_->OnIncomingCapturedData(data, length, 
                                  ...
                                  0); // frame_feedback_id
#else
    FILE *fp=fopen("/tmp/h264_video.mp4","ab");
//    FILE *fp=fopen("/tmp/yuyv_video.raw","ab");
//    FILE *fp=fopen("/tmp/mjpeg_video.mjpeg","ab");
...
Status   VizioSDKAPIWrapper::OpenCamera(const std::string& id, const CameraParameters& params)
{
    sdkApi->OpenCamera(id, params);
    return 0;
}

Status   VizioSDKAPIWrapper::CloseCamera(const std::string& id)
{
    return 0;
}
