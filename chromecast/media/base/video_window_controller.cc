 ...
 public:
  ...
  void SetGeometry(const chromecast::RectF& display_rect,
	...
	task_runner_->PostDelayedTask(
      ...
      LOG(INFO) << " SetGeometry call was very quick. Delaying it.";
    ...
    if(cma_backend_->GetVideoWindow()) {
      ...
      LOG(INFO) << " SetGeometry called on VideoWindow.";
    } else
      LOG(ERROR) << " VideoWindow not available from CMABackend";
...
void VideoWindowController::MaybeRunSetGeometry() {
  if (is_paused_) {
    LOG(INFO) << "All VideoWindow SetGeometry calls are paused. Ignoring request.";
    return;
  }
  ...
  LOG(INFO) << " Scheduling SetGeometry for backend.";
...
void VideoWindowController::ClearVideoWindowGeometry() {
  LOG(INFO) << "VideoWindowController::" << __func__;
  have_video_window_geometry_ = false;
