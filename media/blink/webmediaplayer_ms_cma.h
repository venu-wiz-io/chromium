...
#ifndef MEDIA_BLINK_WEBMEDIAPLAYER_MS_CMA_H_
#define MEDIA_BLINK_WEBMEDIAPLAYER_MS_CMA_H_
...
// Encrypted Media.
class MEDIA_BLINK_EXPORT WebMediaPlayerMsCma
 ...
 public:
  ...
  void TrackAdded(const blink::WebString& track_id) override;
  void TrackRemoved(const blink::WebString& track_id) override;
 ...
 private:
  ...
  void DoLoad(LoadType load_type, const blink::WebMediaPlayerSource& srouce,
              CorsMode cors_mode);
  ...
  std::unique_ptr<blink::MediaStreamRendererFactory> renderer_factory_;
  ...
  // Inner class used for transfering frames on compositor thread to
   ...
   scoped_refptr<blink::WebMediaStreamVideoRenderer> video_frame_provider_;
  ...
  DISALLOW_COPY_AND_ASSIGN(WebMediaPlayerMsCma);
};

}  // namespace media

#endif  // MEDIA_BLINK_WEBMEDIAPLAYER_MS_CMA_H_
