...
void VideoPlaneController::ClearVideoPlaneGeometry() {
  LOG(INFO) << "VideoPlaneController::" << __func__;
  have_video_plane_geometry_ = false;
  video_plane_display_rect_ = RectF(0, 0);
  video_plane_transform_ = VideoPlane::TRANSFORM_NONE;
}

void VideoPlaneController::AddVideoWindow(
          const base::UnguessableToken& plane_id, VideoWindowController* vwc) {
  std::string key = plane_id.ToString();
...
void VideoPlaneController::RemoveVideoWindow(
