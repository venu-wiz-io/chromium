...
#include "third_party/blink/public/platform/modules/mediastream/web_media_stream_track_helper.h"
...
const char kWatchTimeHistogram[] = "Media.WebMediaPlayerMsCma.WatchTime";
...
void SetSinkIdOnMediaThread(
    scoped_refptr<blink::WebMediaStreamAudioRenderer> msSink,
...
bool IsBackgroundSuspendEnabled(const WebMediaPlayerMsCma* wmpi) {
...
bool IsPlayableTrack(const blink::WebMediaStreamTrack& track) {
  return track.Source().GetReadyState() !=
...
class WebMediaPlayerMsCma::FrameDeliverer {
 public:
  typedef base::Callback<void(scoped_refptr<media::VideoFrame>)> RepaintCB;

  FrameDeliverer(const base::WeakPtr<WebMediaPlayerMsCma>& player,
 ...
 private:
  ...
  void EnqueueFrame(const scoped_refptr<media::VideoFrame>& frame) {
...

    {
      ...
      if (tracing_enabled) {
        if (!frame->metadata()->reference_time->is_null()) {
  ...
  const base::WeakPtr<WebMediaPlayerMsCma> player_;
...
WebMediaPlayerMsCma::WebMediaPlayerMsCma(blink::WebLocalFrame* frame,
    ...
    : frame_(frame),
      ...
      renderer_factory_(std::make_unique<blink::MediaStreamRendererFactory>()),
      ...
      simple_watch_timer_(
          base::BindRepeating(&WebMediaPlayerMsCma::OnSimpleWatchTimerTick,
                              base::Unretained(this)),
          base::BindRepeating(&WebMediaPlayerMsCma::GetCurrentTimeInternal,
                              base::Unretained(this))),
  ...
  auto pipeline = std::make_unique<PipelineImpl>(
      ...
      base::BindRepeating(&WebMediaPlayerMsCma::CreateRenderer,
                          base::Unretained(this)),
      media_log_.get());

  pipeline_controller_ = std::make_unique<PipelineController>(
      std::move(pipeline),
      base::BindRepeating(&WebMediaPlayerMsCma::OnPipelineSeeked, weak_this_),
      base::BindRepeating(&WebMediaPlayerMsCma::OnPipelineSuspended, weak_this_),
      base::BindRepeating(&WebMediaPlayerMsCma::OnBeforePipelineResume,
                          weak_this_),
      base::BindRepeating(&WebMediaPlayerMsCma::OnPipelineResumed, weak_this_),
      base::BindRepeating(&WebMediaPlayerMsCma::OnError, weak_this_));

  buffered_data_source_host_ = std::make_unique<BufferedDataSourceHostImpl>(
      base::BindRepeating(&WebMediaPlayerMsCma::OnProgress, weak_this_),
      tick_clock_);
  ...
  auto on_audio_source_provider_set_client_callback = base::BindOnce(
      [](base::WeakPtr<WebMediaPlayerMsCma> self,
  ...
  main_thread_mem_dumper_ = std::make_unique<MemoryDumpProviderProxy>(
      ...
      base::BindRepeating(&WebMediaPlayerMsCma::OnMainThreadMemoryDump,
  ...
  renderer_factory_selector_->SetRemotePlayStateChangeCB(
      BindToCurrentLoop(base::BindRepeating(
          &WebMediaPlayerMsCma::OnRemotePlayStateChange, weak_this_)));
...
WebMediaPlayer::LoadTiming WebMediaPlayerMsCma::Load(
    LoadType load_type,
    const blink::WebMediaPlayerSource& source,
    CorsMode cors_mode) {
  ...
  if (defer_load_cb_) {
    is_deferred = defer_load_cb_.Run(base::BindOnce(
        &WebMediaPlayerMsCma::DoLoad, weak_this_, load_type, source, cors_mode));
  } else {
...
void WebMediaPlayerMsCma::OnWebLayerUpdated() {}

void WebMediaPlayerMsCma::RegisterContentsLayer(cc::Layer* layer) {
  DCHECK(bridge_);
  bridge_->SetContentsOpaque(opaque_);
  client_->SetCcLayer(layer);
}

void WebMediaPlayerMsCma::UnregisterContentsLayer(cc::Layer* layer) {
  // |client_| will unregister its cc::Layer if given a nullptr.
  client_->SetCcLayer(nullptr);
}

void WebMediaPlayerMsCma::OnSurfaceIdUpdated(viz::SurfaceId surface_id) {
...
void WebMediaPlayerMsCma::EnableOverlay() {
  overlay_enabled_ = true;
  if (request_routing_token_cb_ &&
      overlay_mode_ == OverlayMode::kUseAndroidOverlay) {
    overlay_routing_token_is_pending_ = true;
    token_available_cb_.Reset(
        base::BindOnce(&WebMediaPlayerMsCma::OnOverlayRoutingToken, weak_this_));
...
void WebMediaPlayerMsCma::DisableOverlay() {
...
void WebMediaPlayerMsCma::EnteredFullscreen() {
...
void WebMediaPlayerMsCma::ExitedFullscreen() {
...
void WebMediaPlayerMsCma::BecameDominantVisibleContent(bool is_dominant) {
  if (observer_)
    observer_->OnBecameDominantVisibleContent(is_dominant);
}

void WebMediaPlayerMsCma::SetIsEffectivelyFullscreen(
...
void WebMediaPlayerMsCma::OnHasNativeControlsChanged(bool has_native_controls) {
...
void WebMediaPlayerMsCma::OnDisplayTypeChanged(
...
void WebMediaPlayerMsCma::DoLoad(LoadType load_type,
                                const blink::WebMediaPlayerSource& source,
                                CorsMode cors_mode) {
  TRACE_EVENT1("media", "WebMediaPlayerMsCma::DoLoad", "id", media_log_->id());
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());


  bool is_web_stream = false;
  if (!web_stream_.IsNull()) {
    is_web_stream = true;
  }
  ...
  std::string stream_id = is_web_stream ? web_stream_.Id().Utf8() :
      source.GetAsURL().GetString().Substring(0, kMaxUrlLength + 1).Utf8();
  media_log_->AddEvent<MediaLogEvent::kLoad>(stream_id);
  ...
  if (demuxer_override_ || load_type == kLoadTypeMediaSource) {
    ...
    mb_data_source_ = new MultibufferDataSource(
        ...
        base::BindRepeating(&WebMediaPlayerMsCma::NotifyDownloading,
                            weak_this_));
    data_source_.reset(mb_data_source_);
    mb_data_source_->OnRedirect(base::BindRepeating(
        &WebMediaPlayerMsCma::OnDataSourceRedirected, weak_this_));
    ...
    mb_data_source_->Initialize(
        base::BindOnce(&WebMediaPlayerMsCma::DataSourceInitialized, weak_this_));
  }
}

void WebMediaPlayerMsCma::TrackAdded(const blink::WebString& /*track_id*/) {
  Reload();
}

void WebMediaPlayerMsCma::TrackRemoved(const blink::WebString& /*track_id*/) {
  Reload();
}

void WebMediaPlayerMsCma::ActiveStateChanged(bool is_active) {
...
void WebMediaPlayerMsCma::Reload() {
//  DCHECK(thread_checker_.CalledOnValidThread());
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  if (web_stream_.IsNull())
    return;
...
void WebMediaPlayerMsCma::ReloadVideo() {
//  DCHECK(thread_checker_.CalledOnValidThread());
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(!web_stream_.IsNull());

  blink::WebVector<blink::WebMediaStreamTrack> video_tracks =
      blink::WebMediaStreamHelper::VideoTracks(web_stream_);
...
//  } else if ((blink::WebMediaStreamTrackHelper::GetId(video_tracks[0]) !=
//             current_video_track_id_) && IsPlayableTrack(video_tracks[0])) {
//    renderer_action = RendererReloadAction::NEW_RENDERER;
//    current_video_track_id_ = blink::WebMediaStreamTrackHelper::GetId(
//                                video_tracks[0]);
//  }
  } else {
    auto track_id = blink::WebMediaStreamTrackHelper::GetId(video_tracks[0]);
    if (track_id != current_video_track_id_ &&
  ...
  switch (renderer_action) {
    case RendererReloadAction::NEW_RENDERER:
      ...
      video_frame_provider_ = renderer_factory_->GetVideoRenderer(
          web_stream_, frame_deliverer_->GetRepaintCallback(),
...
void WebMediaPlayerMsCma::ReloadAudio() {
//  DCHECK(thread_checker_.CalledOnValidThread());
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(!web_stream_.IsNull());
...
//  SendLogMessage(String::Format("%s()", __func__));
  ...
  RendererReloadAction renderer_action = static_cast<RendererReloadAction>(
                             blink::WebMediaStreamHelper::GetRenderActionAndId(
                   web_stream_, current_audio_track_id_, !!(audio_renderer_)));
...
void WebMediaPlayerMsCma::Pause() {
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::Seek(double seconds) {
...
void WebMediaPlayerMsCma::DoSeek(base::TimeDelta time, bool time_updated) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT2("media", "WebMediaPlayerMsCma::DoSeek", "target",
  ...
  if (paused_ && pipeline_controller_->IsStable() &&
    ...
    if (old_state == kReadyStateHaveEnoughData) {
      main_task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&WebMediaPlayerMsCma::OnBufferingStateChange,
...
void WebMediaPlayerMsCma::SetRate(double rate) {
...
void WebMediaPlayerMsCma::SetVolume(double volume) {
...
void WebMediaPlayerMsCma::SetLatencyHint(double seconds) {
...
void WebMediaPlayerMsCma::SetSecondary(bool secondary)
{
...
void WebMediaPlayerMsCma::SetPreservesPitch(bool preserves_pitch) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  pipeline_controller_->SetPreservesPitch(preserves_pitch);
}

void WebMediaPlayerMsCma::OnRequestPictureInPicture() {
  if (!surface_layer_for_video_enabled_)
...
void WebMediaPlayerMsCma::SetSinkId(
    const blink::WebString& sink_id,
  ...
  media_task_runner_->PostTask(
      FROM_HERE, base::BindOnce(&SetSinkIdOnMediaThread, audio_renderer_,
         audio_source_provider_, sink_id_utf8, std::move(callback)));
...
void WebMediaPlayerMsCma::SetPreload(WebMediaPlayer::Preload preload) {
...
bool WebMediaPlayerMsCma::HasVideo() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  return pipeline_metadata_.has_video;
}

bool WebMediaPlayerMsCma::HasAudio() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  return (audio_renderer_.get() || pipeline_metadata_.has_audio);
}

void WebMediaPlayerMsCma::EnabledAudioTracksChanged(
...
void WebMediaPlayerMsCma::SelectedVideoTrackChanged(
...
gfx::Size WebMediaPlayerMsCma::NaturalSize() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  return pipeline_metadata_.natural_size;
}

gfx::Size WebMediaPlayerMsCma::VisibleSize() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
bool WebMediaPlayerMsCma::Paused() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return pipeline_controller_->GetPlaybackRate() == 0.0f;
}

bool WebMediaPlayerMsCma::PausedWhenHidden() const {
  return paused_when_hidden_;
}

bool WebMediaPlayerMsCma::Seeking() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
double WebMediaPlayerMsCma::Duration() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
double WebMediaPlayerMsCma::timelineOffset() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
base::TimeDelta WebMediaPlayerMsCma::GetCurrentTimeInternal() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
double WebMediaPlayerMsCma::CurrentTime() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
bool WebMediaPlayerMsCma::IsEnded() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return ended_;
}

WebMediaPlayer::NetworkState WebMediaPlayerMsCma::GetNetworkState() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return network_state_;
}

WebMediaPlayer::ReadyState WebMediaPlayerMsCma::GetReadyState() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return ready_state_;
}

blink::WebMediaPlayer::SurfaceLayerMode
WebMediaPlayerMsCma::GetVideoSurfaceLayerMode() const {
  return surface_layer_mode_;
}

blink::WebString WebMediaPlayerMsCma::GetErrorMessage() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return blink::WebString::FromUTF8(media_log_->GetErrorMessage());
}

blink::WebTimeRanges WebMediaPlayerMsCma::Buffered() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
blink::WebTimeRanges WebMediaPlayerMsCma::Seekable() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
bool WebMediaPlayerMsCma::IsPrerollAttemptNeeded() {
...
bool WebMediaPlayerMsCma::DidLoadingProgress() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::Paint(cc::PaintCanvas* canvas,
                               const blink::WebRect& rect,
                               cc::PaintFlags& flags,
                               int already_uploaded_id,
                               VideoFrameUploadMetadata* out_metadata) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("media", "WebMediaPlayerMsCma:paint");
...
bool WebMediaPlayerMsCma::WouldTaintOrigin() const {
...
double WebMediaPlayerMsCma::MediaTimeForTimeValue(double timeValue) const {
  return base::TimeDelta::FromSecondsD(timeValue).InSecondsF();
}

unsigned WebMediaPlayerMsCma::DecodedFrameCount() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return GetPipelineStatistics().video_frames_decoded;
}

unsigned WebMediaPlayerMsCma::DroppedFrameCount() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return GetPipelineStatistics().video_frames_dropped;
}

uint64_t WebMediaPlayerMsCma::AudioDecodedByteCount() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return GetPipelineStatistics().audio_bytes_decoded;
}

uint64_t WebMediaPlayerMsCma::VideoDecodedByteCount() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  return GetPipelineStatistics().video_bytes_decoded;
}

bool WebMediaPlayerMsCma::HasAvailableVideoFrame() const {
  return has_first_frame_;
}

bool WebMediaPlayerMsCma::CopyVideoTextureToPlatformTexture(
    ...
    VideoFrameUploadMetadata* out_metadata) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("media", "WebMediaPlayerMsCma:copyVideoTextureToPlatformTexture");
...
bool WebMediaPlayerMsCma::PrepareVideoFrameForWebGL(
    gpu::gles2::GLES2Interface* gl,
    unsigned target,
    unsigned texture,
    int already_uploaded_id,
  ...
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("media", "WebMediaPlayerMsCma::PrepareVideoFrameForWebGL");
...
// static
void WebMediaPlayerMsCma::ComputeFrameUploadMetadata(
    VideoFrame* frame,
    int already_uploaded_id,
    VideoFrameUploadMetadata* out_metadata) {
  DCHECK(out_metadata);
...
void WebMediaPlayerMsCma::SetContentDecryptionModule(
...
void WebMediaPlayerMsCma::OnEncryptedMediaInitData(
...
void WebMediaPlayerMsCma::OnFFmpegMediaTracksUpdated(
...
void WebMediaPlayerMsCma::SetCdmInternal(
    blink::WebContentDecryptionModule* cdm) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(cdm);
  ...
  pipeline_controller_->SetCdm(
      cdm_context,
      base::BindOnce(&WebMediaPlayerMsCma::OnCdmAttached, weak_this_));
}

void WebMediaPlayerMsCma::OnCdmAttached(bool success) {
...
void WebMediaPlayerMsCma::OnAudioRenderErrorCallback() {
...
void WebMediaPlayerMsCma::OnPipelineSeeked(bool time_updated) {
  TRACE_EVENT2("media", "WebMediaPlayerMsCma::OnPipelineSeeked", "target",
...
void WebMediaPlayerMsCma::OnPipelineSuspended() {
...
void WebMediaPlayerMsCma::OnBeforePipelineResume() {
...
void WebMediaPlayerMsCma::OnPipelineResumed() {
  is_pipeline_resuming_ = false;

  UpdateBackgroundVideoOptimizationState();
}

void WebMediaPlayerMsCma::OnDemuxerOpened() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  if (!web_stream_.IsNull()) {
    FrameDemuxer *demuxer = static_cast<FrameDemuxer*>(demuxer_.get());
    //base::WeakPtr<blink::WebMediaPlayer> weak
    frame_deliverer_.reset(new WebMediaPlayerMsCma::FrameDeliverer(
                             weak_this_,
                             base::BindRepeating(&FrameDemuxer::EnqueueFrame,
                                                 base::Unretained(demuxer)),
                             media_task_runner_));
...
void WebMediaPlayerMsCma::OnMemoryPressure(
...
void WebMediaPlayerMsCma::OnError(PipelineStatus status) {
  ...
  if (found_hls && mb_data_source_) {
    ...
    UMA_HISTOGRAM_BOOLEAN("Media.WebMediaPlayerMsCma.HLS.IsCorsCrossOrigin",
                          mb_data_source_->IsCorsCrossOrigin());
    if (mb_data_source_->IsCorsCrossOrigin()) {
      UMA_HISTOGRAM_BOOLEAN("Media.WebMediaPlayerMsCma.HLS.HasAccessControl",
    ...
    UMA_HISTOGRAM_BOOLEAN(
        "Media.WebMediaPlayerMsCma.HLS.IsMixedContent",
    ...
    media_task_runner_->PostTask(
        ...
        BindToCurrentLoop(base::BindOnce(
            ...
            base::BindOnce(&WebMediaPlayerMsCma::StartPipeline, weak_this_))));
...
void WebMediaPlayerMsCma::OnEnded() {
  TRACE_EVENT2("media", "WebMediaPlayerMsCma::OnEnded", "duration", Duration(),
               "id", media_log_->id());
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnMetadata(const PipelineMetadata& metadata) {
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::ActivateSurfaceLayerForVideo() {
  ...
  // WebMediaPlayerMsCma. The new player needs to send its id, size and
...
void WebMediaPlayerMsCma::OnBufferingStateChange(
    BufferingState state,
    BufferingStateChangeReason reason) {
  OnBufferingStateChangeInternal(state, reason, false);
}

void WebMediaPlayerMsCma::CreateVideoDecodeStatsReporter() {
  ...
  video_decode_stats_reporter_ = std::make_unique<VideoDecodeStatsReporter>(
      std::move(recorder),
      base::BindRepeating(&WebMediaPlayerMsCma::GetPipelineStatistics,
                          base::Unretained(this)),
...
void WebMediaPlayerMsCma::OnProgress() {
  DVLOG(4) << __func__;
...
bool WebMediaPlayerMsCma::CanPlayThrough() {
...
void WebMediaPlayerMsCma::OnBufferingStateChangeInternal(
    BufferingState state,
    BufferingStateChangeReason reason,
  ...
  if (state == BUFFERING_HAVE_ENOUGH) {
    TRACE_EVENT1("media", "WebMediaPlayerMsCma::BufferingHaveEnough", "id",
                 media_log_->id());
...
void WebMediaPlayerMsCma::OnDurationChange() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnAddTextTrack(const TextTrackConfig& config,
...
void WebMediaPlayerMsCma::OnWaiting(WaitingReason reason) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  ...
  switch (reason) {
    ...
    // picture on how to refactor WebMediaPlayerMsCma, PipelineController and
    // PipelineImpl.
...
void WebMediaPlayerMsCma::OnVideoNaturalSizeChange(const gfx::Size& size) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK_NE(ready_state_, WebMediaPlayer::kReadyStateHaveNothing);

  TRACE_EVENT0("media", "WebMediaPlayerMsCma::OnVideoNaturalSizeChange");
...
void WebMediaPlayerMsCma::OnVideoOpacityChange(bool opaque) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK_NE(ready_state_, WebMediaPlayer::kReadyStateHaveNothing);
...
void WebMediaPlayerMsCma::OnVideoFrameRateChange(base::Optional<int> fps) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  if (power_status_helper_)
...
void WebMediaPlayerMsCma::OnAudioConfigChange(const AudioDecoderConfig& config) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK_NE(ready_state_, WebMediaPlayer::kReadyStateHaveNothing);
...
void WebMediaPlayerMsCma::OnVideoConfigChange(const VideoDecoderConfig& config) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK_NE(ready_state_, WebMediaPlayer::kReadyStateHaveNothing);
...
void WebMediaPlayerMsCma::OnVideoAverageKeyframeDistanceUpdate() {
  UpdateBackgroundVideoOptimizationState();
}

void WebMediaPlayerMsCma::OnAudioDecoderChange(const PipelineDecoderInfo& info) {
...
void WebMediaPlayerMsCma::OnVideoDecoderChange(const PipelineDecoderInfo& info) {
...
void WebMediaPlayerMsCma::OnFrameHidden() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnFrameClosed() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  UpdatePlayState();
}

void WebMediaPlayerMsCma::OnFrameShown() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  background_pause_timer_.Stop();
...
void WebMediaPlayerMsCma::OnIdleTimeout() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnPlay() {
  client_->RequestPlay();
}

void WebMediaPlayerMsCma::OnPause() {
  client_->RequestPause();
}

void WebMediaPlayerMsCma::OnMuted(bool muted) {
  client_->RequestMuted(muted);
}

void WebMediaPlayerMsCma::OnSeekForward(double seconds) {
  DCHECK_GE(seconds, 0) << "Attempted to seek by a negative number of seconds";
  client_->RequestSeek(CurrentTime() + seconds);
}

void WebMediaPlayerMsCma::OnSeekBackward(double seconds) {
  DCHECK_GE(seconds, 0) << "Attempted to seek by a negative number of seconds";
  client_->RequestSeek(CurrentTime() - seconds);
}

void WebMediaPlayerMsCma::OnEnterPictureInPicture() {
  client_->RequestEnterPictureInPicture();
}

void WebMediaPlayerMsCma::OnExitPictureInPicture() {
  client_->RequestExitPictureInPicture();
}

void WebMediaPlayerMsCma::OnSetAudioSink(const std::string& sink_id) {
  SetSinkId(WebString::FromASCII(sink_id),
            base::DoNothing::Once<base::Optional<blink::WebSetSinkIdError>>());
}

void WebMediaPlayerMsCma::OnVolumeMultiplierUpdate(double multiplier) {
  volume_multiplier_ = multiplier;
  SetVolume(volume_);
}

void WebMediaPlayerMsCma::OnBecamePersistentVideo(bool value) {
  client_->OnBecamePersistentVideo(value);
  overlay_info_.is_persistent_video = value;
  MaybeSendOverlayInfoToDecoder();
}

void WebMediaPlayerMsCma::OnPowerExperimentState(bool state) {
  if (power_status_helper_)
    power_status_helper_->UpdatePowerExperimentState(state);
}

void WebMediaPlayerMsCma::ScheduleRestart() {
...
void WebMediaPlayerMsCma::RequestRemotePlaybackDisabled(bool disabled) {
  if (observer_)
    observer_->OnRemotePlaybackDisabled(disabled);
}

#if defined(OS_ANDROID)
void WebMediaPlayerMsCma::FlingingStarted() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::FlingingStopped() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(disable_pipeline_auto_suspend_);
...
void WebMediaPlayerMsCma::OnRemotePlayStateChange(MediaStatus::State state) {
  DCHECK(is_flinging_);
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::SetPoster(const blink::WebURL& poster) {
  has_poster_ = !poster.IsEmpty();
}

void WebMediaPlayerMsCma::DataSourceInitialized(bool success) {
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnDataSourceRedirected() {
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(mb_data_source_);
...
void WebMediaPlayerMsCma::NotifyDownloading(bool is_downloading) {
...
void WebMediaPlayerMsCma::OnOverlayRoutingToken(
...
void WebMediaPlayerMsCma::OnOverlayInfoRequested(
...
void WebMediaPlayerMsCma::MaybeSendOverlayInfoToDecoder() {
...
std::unique_ptr<Renderer> WebMediaPlayerMsCma::CreateRenderer(
  ...
  request_overlay_info_cb = BindToCurrentLoop(base::BindRepeating(
      &WebMediaPlayerMsCma::OnOverlayInfoRequested, weak_this_));
#endif
...
void WebMediaPlayerMsCma::StartPipeline() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  ...
  Demuxer::EncryptedMediaInitDataCB encrypted_media_init_data_cb =
      BindToCurrentLoop(base::BindRepeating(
          &WebMediaPlayerMsCma::OnEncryptedMediaInitData, weak_this_));
  ...
  vfc_task_runner_->PostTask(
      ...
      base::BindOnce(&VideoFrameCompositor::SetOnNewProcessedFrameCallback,
                     base::Unretained(compositor_.get()),
                     BindToCurrentLoop(base::BindOnce(
                         &WebMediaPlayerMsCma::OnFirstFrame, weak_this_))));
  ...
  if (demuxer_override_) {
    ...
    FrameDemuxer* frame_demuxer = new FrameDemuxer(
        BindToCurrentLoop(
            base::Bind(&WebMediaPlayerMsCma::OnDemuxerOpened, weak_this_)),
        BindToCurrentLoop(
            base::Bind(&WebMediaPlayerMsCma::OnProgress, weak_this_)),
        media_log_.get());

    demuxer_.reset(frame_demuxer);
    ...
    Demuxer::MediaTracksUpdatedCB media_tracks_updated_cb =
        BindToCurrentLoop(base::BindRepeating(
            &WebMediaPlayerMsCma::OnFFmpegMediaTracksUpdated, weak_this_));
    ...
    chunk_demuxer_ =
        new ChunkDemuxer(BindToCurrentLoop(base::BindOnce(
                             &WebMediaPlayerMsCma::OnDemuxerOpened, weak_this_)),
                         BindToCurrentLoop(base::BindRepeating(
                             &WebMediaPlayerMsCma::OnProgress, weak_this_)),
    ...
    if (base::FeatureList::IsEnabled(kMemoryPressureBasedSourceBufferGC)) {
      ...
      memory_pressure_listener_ =
          std::make_unique<base::MemoryPressureListener>(
              FROM_HERE, base::Bind(&WebMediaPlayerMsCma::OnMemoryPressure,
                                    base::Unretained(this)));
    }
  }
...
void WebMediaPlayerMsCma::RepaintInternal() {
  DVLOG(1) << __func__;
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  get_client()->Repaint();
}

void WebMediaPlayerMsCma::OnSourceError() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  SetNetworkState(WebMediaPlayer::kNetworkStateFormatError);
  RepaintInternal();
}

void WebMediaPlayerMsCma::SetNetworkState(WebMediaPlayer::NetworkState state) {
...
void WebMediaPlayerMsCma::SetReadyState(WebMediaPlayer::ReadyState state) {
  DVLOG(1) << __func__ << "(" << state << ")";
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
WebMediaPlayerMsCma::GetAudioSourceProvider() {
  return audio_source_provider_;
}

scoped_refptr<VideoFrame> WebMediaPlayerMsCma::GetCurrentFrameFromCompositor()
    const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  TRACE_EVENT0("media", "WebMediaPlayerMsCma::GetCurrentFrameFromCompositor");
...
void WebMediaPlayerMsCma::UpdatePlayState() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnTimeUpdate() {
...
void WebMediaPlayerMsCma::SetDelegateState(DelegateState new_state,
                                          bool is_idle) {
  DCHECK(delegate_);
...
void WebMediaPlayerMsCma::SetMemoryReportingState(
  ...
  if (is_memory_reporting_enabled) {
    memory_usage_reporting_timer_.Start(FROM_HERE,
                                        ...
                                        &WebMediaPlayerMsCma::ReportMemoryUsage);
  } else {
...
void WebMediaPlayerMsCma::SetSuspendState(bool is_suspended) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
WebMediaPlayerMsCma::PlayState
WebMediaPlayerMsCma::UpdatePlayState_ComputePlayState(bool is_flinging,
...
void WebMediaPlayerMsCma::SetDemuxer(std::unique_ptr<Demuxer> demuxer) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
  DCHECK(!demuxer_);
  ...
  // thread, but before doing it ~WebMediaPlayerMsCma() posts a media thread task
  ...
  media_thread_mem_dumper_ = std::make_unique<MemoryDumpProviderProxy>(
      ...
      base::BindRepeating(&WebMediaPlayerMsCma::OnMediaThreadMemoryDump,
                          media_log_->id(), base::Unretained(demuxer_.get())));
}

void WebMediaPlayerMsCma::ReportMemoryUsage() {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  // About base::Unretained() usage below: We destroy |demuxer_| on the main
  // thread.  Before that, however, ~WebMediaPlayerMsCma() posts a task to the
  ...
  if (demuxer_ && !IsNetworkStateError(network_state_)) {
    base::PostTaskAndReplyWithResult(
        ...
        base::BindOnce(&WebMediaPlayerMsCma::FinishMemoryUsageReport,
                       weak_this_));
  } else {
...
void WebMediaPlayerMsCma::FinishMemoryUsageReport(int64_t demuxer_memory_usage) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::OnMainThreadMemoryDump(
    int32_t id,
...
// static
void WebMediaPlayerMsCma::OnMediaThreadMemoryDump(
    int32_t id,
    Demuxer* demuxer,
    const base::trace_event::MemoryDumpArgs& args,
    base::trace_event::ProcessMemoryDump* pmd) {
...
void WebMediaPlayerMsCma::ScheduleIdlePauseTimer() {
  ...
  background_pause_timer_.Start(FROM_HERE, base::TimeDelta::FromSeconds(5),
                                this, &WebMediaPlayerMsCma::OnPause);
}

void WebMediaPlayerMsCma::CreateWatchTimeReporter() {
...
void WebMediaPlayerMsCma::UpdateSecondaryProperties() {
...
bool WebMediaPlayerMsCma::IsHidden() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  return delegate_->IsFrameHidden() && !delegate_->IsFrameClosed();
}

bool WebMediaPlayerMsCma::IsStreaming() const {
  return data_source_ && data_source_->IsStreaming();
}

bool WebMediaPlayerMsCma::DoesOverlaySupportMetadata() const {
  return pipeline_metadata_.video_decoder_config.video_transformation() ==
         kNoTransformation;
}

void WebMediaPlayerMsCma::UpdateRemotePlaybackCompatibility(bool is_compatible) {
  DCHECK(main_task_runner_->BelongsToCurrentThread());

  client_->RemotePlaybackCompatibilityChanged(loaded_url_, is_compatible);
}

void WebMediaPlayerMsCma::ForceStaleStateForTesting(ReadyState target_state) {
  stale_state_override_for_testing_.emplace(target_state);
  UpdatePlayState();
}

bool WebMediaPlayerMsCma::IsSuspendedForTesting() {
...
bool WebMediaPlayerMsCma::DidLazyLoad() const {
  return did_lazy_load_;
}

void WebMediaPlayerMsCma::OnBecameVisible() {
  have_enough_after_lazy_load_cb_.Cancel();
  needs_first_frame_ = !has_first_frame_;
  UpdatePlayState();
}

bool WebMediaPlayerMsCma::IsOpaque() const {
  return opaque_;
}

int WebMediaPlayerMsCma::GetDelegateId() {
  return delegate_id_;
}

base::Optional<viz::SurfaceId> WebMediaPlayerMsCma::GetSurfaceId() {
  if (!surface_layer_for_video_enabled_)
    return base::nullopt;
  return bridge_->GetSurfaceId();
}

void WebMediaPlayerMsCma::RequestVideoFrameCallback() {
  compositor_->SetOnFramePresentedCallback(BindToCurrentLoop(base::BindOnce(
      &WebMediaPlayerMsCma::OnNewFramePresentedCallback, weak_this_)));
}

void WebMediaPlayerMsCma::OnNewFramePresentedCallback() {
  client_->OnRequestVideoFrameCallback();
}

std::unique_ptr<blink::WebMediaPlayer::VideoFramePresentationMetadata>
WebMediaPlayerMsCma::GetVideoFramePresentationMetadata() {
  return compositor_->GetLastPresentedFrameMetadata();
}

void WebMediaPlayerMsCma::UpdateFrameIfStale() {
  // base::Unretained is safe here because |compositor_| is destroyed on
  // |vfc_task_runner_|. The destruction is queued from |this|' destructor,
  // which also runs on |main_task_runner_|, which makes it impossible for
  // UpdateCurrentFrameIfStale() to be queued after |compositor_|'s dtor.
...
base::WeakPtr<blink::WebMediaPlayer> WebMediaPlayerMsCma::AsWeakPtr() {
  return weak_this_;
}

bool WebMediaPlayerMsCma::ShouldPausePlaybackWhenHidden() const {
...
bool WebMediaPlayerMsCma::ShouldDisableVideoWhenHidden() const {
...
bool WebMediaPlayerMsCma::IsBackgroundOptimizationCandidate() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::UpdateBackgroundVideoOptimizationState() {
  if (IsHidden()) {
    if (ShouldPausePlaybackWhenHidden()) {
      ...
      update_background_status_cb_.Reset(
          base::Bind(&WebMediaPlayerMsCma::DisableVideoTrackIfNeeded,
                     base::Unretained(this)));
...
void WebMediaPlayerMsCma::PauseVideoIfNeeded() {
  DCHECK(IsHidden());
...
void WebMediaPlayerMsCma::EnableVideoTrackIfNeeded() {
...
void WebMediaPlayerMsCma::DisableVideoTrackIfNeeded() {
  DCHECK(IsHidden());
...
void WebMediaPlayerMsCma::SetPipelineStatisticsForTest(
    const PipelineStatistics& stats) {
  pipeline_statistics_for_test_ = base::make_optional(stats);
}

PipelineStatistics WebMediaPlayerMsCma::GetPipelineStatistics() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::SetPipelineMediaDurationForTest(
    base::TimeDelta duration) {
  pipeline_media_duration_for_test_ = base::make_optional(duration);
}

base::TimeDelta WebMediaPlayerMsCma::GetPipelineMediaDuration() const {
  DCHECK(main_task_runner_->BelongsToCurrentThread());
...
void WebMediaPlayerMsCma::SwitchToRemoteRenderer(
...
void WebMediaPlayerMsCma::SwitchToLocalRenderer(
...
void WebMediaPlayerMsCma::RecordVideoNaturalSize(const gfx::Size& natural_size) {
...
void WebMediaPlayerMsCma::SetTickClockForTest(
    const base::TickClock* tick_clock) {
  tick_clock_ = tick_clock;
  buffered_data_source_host_->SetTickClockForTest(tick_clock);
}

void WebMediaPlayerMsCma::OnFirstFrame(base::TimeTicks frame_time) {
...
void WebMediaPlayerMsCma::RecordTimingUMA(const std::string& key,
...
void WebMediaPlayerMsCma::RecordEncryptionScheme(
...
bool WebMediaPlayerMsCma::IsInPictureInPicture() const {
  DCHECK(client_);
  return client_->DisplayType() ==
         WebMediaPlayer::DisplayType::kPictureInPicture;
}

void WebMediaPlayerMsCma::OnPictureInPictureAvailabilityChanged(bool available) {
  delegate_->DidPictureInPictureAvailabilityChange(delegate_id_, available);
}

void WebMediaPlayerMsCma::MaybeSetContainerNameForMetrics() {
...
void WebMediaPlayerMsCma::MaybeUpdateBufferSizesForPlayback() {
...
void WebMediaPlayerMsCma::OnSimpleWatchTimerTick() {
...
GURL WebMediaPlayerMsCma::GetSrcAfterRedirects() {
  return mb_data_source_ ? mb_data_source_->GetUrlAfterRedirects() : GURL();
}

void WebMediaPlayerMsCma::UpdateSmoothnessHelper() {
...
WebMediaPlayerMsCma::GetLearningTaskController(const char* task_name) {
...
bool WebMediaPlayerMsCma::HasUnmutedAudio() const {
